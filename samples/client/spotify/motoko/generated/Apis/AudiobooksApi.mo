...
module {
    ...
    public func getAnAudiobook(config : Config__, id : Text, market : Text) : async* AudiobookObject {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?AudiobookObject.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (AudiobookObject.fromJSON(jsonValue)) {
                        case (?value) value;
    ...
    public func getMultipleAudiobooks(config : Config__, ids : Text, market : Text) : async* GetMultipleAudiobooks200Response {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?GetMultipleAudiobooks200Response.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (GetMultipleAudiobooks200Response.fromJSON(jsonValue)) {
                        case (?value) value;
