...
module {
    ...
    public func getACategory(config : Config__, categoryId : Text, locale : Text) : async* CategoryObject {
        let {baseUrl; accessToken; cycles} = config;
        let url = baseUrl # "/browse/categories/{category_id}"
            |> Text.replace(_, #text "{category_id}", categoryId)
            # "?" # "locale=" # locale;
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?CategoryObject.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (CategoryObject.fromJSON(jsonValue)) {
                        case (?value) value;
                        case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to CategoryObject");
                    }
                };
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to deserialize response");
            })
    ...
    public func getCategories(config : Config__, locale : Text, limit : Int, offset : Int) : async* GetCategories200Response {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?GetCategories200Response.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (GetCategories200Response.fromJSON(jsonValue)) {
                        case (?value) value;
                        case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to GetCategories200Response");
                    }
                };
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to deserialize response");
            })
