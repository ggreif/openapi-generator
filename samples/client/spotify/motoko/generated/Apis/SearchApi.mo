...
module {
    ...
    public func search(config : Config__, q : Text, type_ : [Text], market : Text, limit : Int, offset : Int, includeExternal : Text) : async* Search200Response {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?Search200Response.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (Search200Response.fromJSON(jsonValue)) {
                        case (?value) value;
                        case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Search200Response");
                    }
                };
